...
namespace   Network
{
    ...
    bool            NetworkManager::send(const std::string &chunk, const std::string &connector_id)
    {
        ...
        if (chunk_size <= _connectors.at(connector_id)->getWriteBuffer().getSpaceLeft())
        {
            ...
            // if (((_fdsetList.at(connector_id)).revents & POLLOUT)) {
            //     std::cerr << "&POLLOUT revent AFTER" << std::endl;
            // }
            return (true);
        }
        std::cerr << "Can't send chunk of size " << chunk_size << " because only " << _connectors.at(connector_id)->getWriteBuffer().getSpaceLeft() << " space left" << std::endl;
        return (false);
    }
    ...
    void            NetworkManager::run()
    {
        ...
        if (poll(_fdset, _fdsetList.size(), 0) > 0)
        {
...

            {
                ...
                // std::cout << "Checking Pollout for is == " << it->first << std::endl;
                if ((it->second.revents & POLLOUT))
                {
                    ...
                    {
                        //std::cerr << "Byte written " << it->first << std::endl;
